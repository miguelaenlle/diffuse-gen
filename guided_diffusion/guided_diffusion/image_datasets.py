...
def load_data(
    *,
    ...
    img_path = bf.join(data_dir, "masked-images")
    mask_path = bf.join(data_dir, "masks")
    
    classes = None
    if class_cond:
        # Assume classes are the first part of the filename,
        # before an underscore.
        class_names = [bf.basename(path).split("_")[0] for path in img_path]
...
class ImageDataset(Dataset):
    def __init__(
        ...
        all_img_files = _list_image_files_recursively(image_path)
        self.mask_path = mask_path
        self.local_images = all_img_files[shard:][::num_shards]
