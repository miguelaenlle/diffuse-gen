...
class TrainLoop:
    ...
    def _load_and_sync_parameters(self):
        resume_checkpoint = find_resume_checkpoint(self.save_prefix, self.resume_checkpoint) or self.resume_checkpoint
    ...
    def _load_ema_parameters(self, rate):
        ...
        main_checkpoint = find_resume_checkpoint(self.save_prefix, self.resume_checkpoint) or self.resume_checkpoint
        ema_checkpoint = find_ema_checkpoint(main_checkpoint, self.save_prefix, self.resume_step, rate)
        if ema_checkpoint:
            if dist.get_rank() == 0:
    ...
    def _load_optimizer_state(self):
        ...
        opt_checkpoint = bf.join(
            bf.dirname(main_checkpoint), f"{self.save_prefix}opt{self.resume_step:06}.pt"
        )
    ...
    def run_loop(self, max_steps=None):
        ...
        while (
            ...
            if max_steps and self.step - self.resume_step > max_steps:
                break
    ...
    def save(self):
        def save_checkpoint(rate, params):
            ...
            if dist.get_rank() == 0:
                ...
                if not rate:
                    filename = f"{self.save_prefix}model{(self.step+self.resume_step):06d}.pt"
                else:
                    filename = f"{self.save_prefix}ema_{rate}_{(self.step+self.resume_step):06d}.pt"
        ...
        if dist.get_rank() == 0:
            with bf.BlobFile(
                bf.join(get_blob_logdir(), f"{self.save_prefix}opt{(self.step+self.resume_step):06d}.pt"),
                "wb",
            ) as f:
...
def find_resume_checkpoint(save_prefix, resume_checkpoint):
    # On your infrastructure, you may want to override this to automatically
    # discover the latest checkpoint on your blob storage, etc.
    if not resume_checkpoint or not save_prefix:
        return None
    split = resume_checkpoint.split("/")
    filename = save_prefix + split[-1]
    split[-1] = filename
    path = "/".join(split)
    return path


def find_ema_checkpoint(main_checkpoint, save_prefix, step, rate):
    if main_checkpoint is None:
        return None
    filename = f"{save_prefix}ema_{rate}_{(step):06d}.pt"
